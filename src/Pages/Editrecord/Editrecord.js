...
const Editrecord = props => {
  ...
  const Update = (values, resetForm, setSubmitting) => {
    fetch(`http://localhost:3000/api/records/${props.item._id}`, {
      method: "PUT",
      body: JSON.stringify({
        cnic: values.cnic,
      ...
      <MDBContainer>
        ...
        <Formik
          ...
          validationSchema={Yup.object({
            cnic: Yup.string()
              .matches(/^[0-9+]{5}-[0-9+]{7}-[0-9]{1}$/, "Provide a valid CNIC")
          ...
          onSubmit={(values, { setSubmitting, resetForm }) => {
            ...
            Update(values, resetForm, setSubmitting);
          }}
        >
          {formik => (
            <MDBContainer>
                    ...
                    <MDBCol md="8">
                      <div>
                        <TextField
                          ...
                          label="Date of Birth"
                          type="date"
                          defaultValue={props.item.dob}
                          onChange={formik.handleChange}
                          onBlur={formik.handleBlur}
                          value={formik.values.editdob}
                          className={classes.fullLength}
                          InputLabelProps={{
                            shrink: true
                          }}
                          helperText={
                            <ErrorMessage name="editdob">
                              {msg => <div style={{ color: "red" }}>{msg}</div>}
                            </ErrorMessage>
                          }
                        />
